...
bug1 <- "model {
  for (i in 1:N) {
  mu[i] <- 
  ifelse(t[i]<cp[1], a[1]+b[1]*t[i], 0) +
  ifelse(t[i]<cp[2], a[2]+b[2]*t[i], 0) * ifelse(t[i]>cp[1], 1, 0) +
  ifelse(t[i]>cp[2], a[3]+b[3]*t[i], 0)
...
mu2 <- (t < med[2])              * f(t, ab2, 1) + 
       (t > med[2] & t < med[3]) * f(t, ab2, 2) + 
       (t > med[3])              * f(t, ab2, 3)
...
**Answer 4-5:** When modeled this way, the intercepts are different from the set intercepts in the data simulation. But, the estimated intercepts  are what I would expect for the three separate linear functions that comprise the data (just by looking at the figure: a = ~1, ~2, ~3.5). I should be able to convert the intercepts a1, a2, and a3 I found using JAGS to the know intercepts, but had difficulties doing so. 
