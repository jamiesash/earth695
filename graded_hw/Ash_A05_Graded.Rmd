...
Score:  74/100. Very good! This was a tough assignment.   

Ex 1. 0/5. You forgot to do this one.  

Ex 2. 14/15. Excellent!   

- On the last line where you say "combine exponents" the second factor on the LHS should be $e^{-\lambda(N+\beta)}$, right?  

- You wrote "Improper density functions are handled by truncating the tails i.e. setting the shape and rate to a very low value such as $\alpha = \beta = 0.001$", but---you will want to remember this---setting the shape and rate to 0.001 does not "truncate the tails". (Truncating the tails would mean multiplying the gamma by `dunif(a,b)` in which $a$ is a small number just greater than zero and $b$ is a very large number.) What setting shape and rate to 0.001 actually does is make the gamma look a lot like the reciprocal density.    

Ex 3. 15/15. Excellent. You are getting good with the LaTeX.  

Ex 4. 15/25. You are on the right track here, and if you'd had more time you probably would have nailed it.  

Ex 4(a). Great, but don't use `*` for multiplication in math. Use `*` for multiplication in code, but in math, the asterisk means either convolution or some other esoteric type of product. When you want to be explicit about multiplication in math use `\cdot` or `\times`.  

Ex 4(b). Nope, sorry. The prior should not contain any data, $y$. And your last line says "finding the likelihood of the prior", which makes no sense, right? There is a likelihood and there is a prior, but there is no likelihood _of_ the prior. On the last line there is a quantity $\sum u_i$ that magically appears.     

Ex 4(c)-(d). Yes, you are reaching here. I didn't help you very much because I meant to say "in the limit as one of the parameters approaches **zero**" instead of "in the limit as one of the parameters approaches infinity". Anyway, see the solutions.  

Ex 5. 30/40.  

- Your `bug1` is on the right track. You took the time to read the JAGS manual where you discovered `ifelse()`. Your priors are perfect, and your observation loop is sort of sensible. However, your expression for `mu[i]` cannot be right because every term has the common factor `t[i]`.  

- In your initial values you have the statements  
```
   a  <- double(3)
   b  <- double(3)
  cp  <- double(2)
```  
But all they do is create storage. You need to give them numerical values.  

- In your `run.jags()` chunk you are monitoring the right quantities. Good!   
