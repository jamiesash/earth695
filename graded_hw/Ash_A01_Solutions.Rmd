...
# Score/Comments {-}  
90/100. Extra credit for creative extra work!  

## Ex 1 {-}  
Excellent work (better than mine). You get extra credit for this problem. You wrote "There is a `xtest*100` percent" which is close, but in order to get the actual number in your narrative you needed to put an "r" after the opening backtick.  Using `round()` to limit the number of digits after the decimal point is also good. See my solutions.    

**(d)** Nice figure! 

**(e)** I agree. When estimating from samples the outer quartiles will always be less accurate than the inner quartiles because the density of samples is much lower out there in the tails.  

## Ex 2 {-}  
Your work may be slightly more accurate than mine here, but the point was just to use the samples. See what I did in the solutions. If I had wanted more accuracy I would have generated more samples of `x` and thus more values of `f(x)`.    

## Ex 3 {-}  
Nice figure and good discussion. The main thing to note is that, even for large CoV, the lognormal has no probability mass near zero, whereas the gamma has a lot there. That could be important in some applications.  

## Ex 4 {-}  
**(a)**  Good but I would use points for `y1 <- expit(logit(x))` so it isn't occluded by the red `y2 <- x` line. See the solutions.  
**(b)**  Your answer here doesn't make sense to me. I would just use `plot(f(x),  g(x)); abline(0, 1, col="red")`.  

## Ex 5 {-}  
This is regression with additive noise, so there is no Poisson distribution in the problem. See the solutions.  

**(c)** Standardization will reduce the correlation between samples of $a$ and $b$ as well as the number of samples needed for given accuracy. Consider: If all your x-y values are way out there in the first quadrant, a change in slope will cause a change in intercept, but if you standardize, a change in slope leaves intercept unchanged or nearly so.  

**(d)** Yes! Using a student-t instead of a normal will make any regression with additive noise more robust to outliers.  

**(e)** Orthogonal polynomials are better. Their coefficients will be less correlated so fewer samples will be less computation will be required to estimate them. The effect is like standardization, and for similar mathematical reasons.  
